...
execute "mariadb-install-drupal-privileges" do
  command "/usr/bin/mysql -h #{node['drupal']['db']['host']} -u root -p#{node['mariadb']['server_root_password']} < /etc/mysql/drupal-grants.sql"
  action :nothing
end
...
template "/etc/mysql/drupal-grants.sql" do
  ...
  notifies :run, "execute[mariadb-install-drupal-privileges]", :immediately
end

execute "create #{node['drupal']['db']['database']} database" do
  command "/usr/bin/mysqladmin -h #{node['drupal']['db']['host']} -u root -p#{node['mariadb']['server_root_password']} create #{node['drupal']['db']['database']}"
  not_if "mysql -h #{node['drupal']['db']['host']} -u root -p#{node['mariadb']['server_root_password']} --silent --skip-column-names --execute=\"show databases like '#{node['drupal']['db']['database']}'\" | grep #{node['drupal']['db']['database']}"
end
